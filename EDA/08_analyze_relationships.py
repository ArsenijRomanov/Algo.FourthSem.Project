...
- при необходимости рассчитывает солнечную геометрию через pvlib;
- выделяет дневные часы по высоте Солнца;
- считает матрицы корреляций:
  - по всем данным;
  - по дневным данным;
- строит графики зависимостей между признаками:
  - scatter для ключевых пар;
  - binned-графики средних значений по бинам;
- сохраняет таблицы и графики в папку 08_relationships.

Что проверяется:
1. Как облачность связана с GHI, DNI и DHI;
2. Как температура связана с радиацией;
3. Как ветер связан с температурой и радиацией;
4. Насколько зависимости отличаются, если анализировать только дневные часы;
5. Нет ли странных комбинаций значений, которые могут говорить о проблемах в данных.
...
- анализ радиации имеет основной смысл именно на дневных данных.
"""
...
ALL_CORR_COLUMNS = NUMERIC_COLUMNS.copy()
...
def compute_correlation_tables(df: pd.DataFrame, cols: list[str]):
    pearson_corr = df[cols].corr(method="pearson").round(4)
    spearman_corr = df[cols].corr(method="spearman").round(4)
...
def plot_binned_relationship(df: pd.DataFrame, x_col: str, y_col: str, out_path: Path, title_suffix: str) -> None:
    ...
    if work_df.empty or work_df[x_col].nunique() < 2:
        print(f"Пропуск binned-графика для {y_col} vs {x_col}: недостаточно уникальных значений.")
        return

    work_df["x_bin"] = pd.cut(
        work_df[x_col],
        bins=N_BINS,
        include_lowest=True,
        duplicates="drop",
    ...
    grouped = (
        work_df.groupby("x_bin", observed=True)[y_col]
        .agg(["mean", "count"])
        .reset_index()
    )
    ...
    print(f"Сохранена таблица бинов: {csv_path}")
...
def main() -> None:
    ...
    plot_correlation_heatmap(
        ...
        dirs["correlations"] / "08_pearson_corr_all_data.png",
    )
    plot_correlation_heatmap(
        spearman_all,
        "Spearman correlation (all data)",
        dirs["correlations"] / "08_spearman_corr_all_data.png",
    )
    print()
    ...
    plot_correlation_heatmap(
        pearson_day,
        "Pearson correlation (daytime data)",
        dirs["correlations"] / "08_pearson_corr_daytime_data.png",
    )
    plot_correlation_heatmap(
        spearman_day,
        "Spearman correlation (daytime data)",
        dirs["correlations"] / "08_spearman_corr_daytime_data.png",
    )
    print()
    ...
    for x_col, y_col in SCATTER_PAIRS:
        out_path = dirs["scatter"] / f"08_daytime_scatter_{y_col}_vs_{x_col}.png"
        plot_scatter_pair(day_df, x_col, y_col, out_path, "daytime")
    print()

    for x_col, y_col in BINNED_PAIRS:
        out_path = dirs["binned"] / f"08_daytime_binned_{y_col}_vs_{x_col}.png"
        plot_binned_relationship(day_df, x_col, y_col, out_path, "daytime")
    print()
